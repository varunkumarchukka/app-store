...
class AppStore extends Component {
  ...
  getSelectedAppList = (searchInput, selectedTabId) => {
    const {initialAppsList} = this.state
    const filteredData = initialAppsList.filter(
      eachItem => eachItem.category === selectedTabId,
    )

    const updatedData = filteredData.filter(eachItem =>
  ...
  render() {
    ...
    const filteredDate = this.getSelectedAppList(searchInput, selectedTabId)
        ...
        <ul className="tab-list-container">
          {tabsList.map(eachTab => (
            <TabItem
              eachTab={eachTab}
              key={eachTab.tabId}
              getUpdatedTabId={this.getUpdatedTabId}
              isActive={eachTab.tabId === selectedTabId}
            />
          ))}
        </ul>

        <ul className="app-list-container">
          {filteredDate.map(eachApp => (
